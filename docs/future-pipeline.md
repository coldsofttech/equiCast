...
`packages/future/Dockerfile` containerizes the CLI. `images.yml`
...
`images.yml` builds and pushes `equicast-future` to GHCR automatically
on changes to `packages/datafeed/`, `packages/metrics/`, or
`packages/future/` on `main` or a `dev/**` release branch (pushing `:dev`
instead of `:latest` on the latter — see
`.github/actions/build-push-image`), or on demand via its
`workflow_dispatch` trigger (Actions tab → *Build Images* → *Run workflow*
→ pick `future` from the `asset_class` dropdown, or `all`).
...
| `tickers` | *(empty)* | Optional `;`-separated list of future keys (e.g. `GOLD;CRUDE_OIL_WTI`) to restrict this run to, instead of every future in the config. When set, `build-catalog` merges this run's futures into the existing catalog instead of replacing it outright |
