...
`packages/watchlist/Dockerfile` containerizes the CLI. `images.yml`
...
`images.yml` builds and pushes `equicast-watchlist` to GHCR automatically
on changes to `packages/datafeed/`, `packages/fx/`, `packages/benchmark/`,
`packages/future/`, or `packages/watchlist/` on `main` or a `dev/**`
...
→ pick `watchlist` from the `asset_class` dropdown, or `all`).
