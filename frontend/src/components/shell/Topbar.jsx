 ...
 * end-to-end proof from Phase 0's identity wiring — see useCurrentUser) as
 * a clearly-labeled pill with a currency icon, theme toggle, and the
 * account menu (avatar, Accounts, Settings, sign-out). `profile`/
 * `setProfile` are passed into UserMenu so its Settings drawer can update
 * the same state this badge reads, instead of each fetching its own copy
 * and drifting out of sync after a save.
 */
function Topbar() {
