...
/** Date/no_of_shares/average_price_native/amount_native form shared by "Add
 * Buy", "Add Dividend", and editing an existing entry of either type —
 * `type` picks which fields show, `mode` picks whether an account/
 * portfolio selector shows (only relevant when creating: editing an
 * existing entry never moves it to a different holding) and whether this
 * calls `onSubmit(holdingId, { type, ...fields })` (create) or
 * `onSubmit(holdingId, fields)` (edit, no `type` — immutable once
 * created). Only ever sends the *native*-currency value the user typed —
 * the backend resolves/stores the converted (default-currency) figure
 * itself (see equicast_core.transactions module docstring), so there's
 * nothing for this form to compute or display for that. */
function TransactionForm({ type, mode, instances, initialValues, onSubmit, onCancel }) {
  const [holdingId, setHoldingId] = useState(
    initialValues?.holdingId ?? instances[0]?.holding.id ?? ""
  );
  const [date, setDate] = useState(initialValues?.date ?? "");
  const [shares, setShares] = useState(initialValues?.no_of_shares ?? "");
  const [averagePrice, setAveragePrice] = useState(initialValues?.average_price_native ?? "");
  ...
  const handleSubmit = (event) => {
    ...
    const fields =
      type === "BUY"
        ? { date, no_of_shares: shares, average_price_native: averagePrice }
    ...
    <form onSubmit={handleSubmit} className="ec-form">
      ...
      {type === "BUY" ? (
        <>
          ...
          />
          <TextField
            id="transaction-average-price"
            label="Average price (native)"
            type="number"
            min="0.01"
            step="0.01"
            required
...
/** One BUY/SELL trade card for TRANSACTION-mode holdings. */
function TradeCard({ transaction, nativeCurrency, location }) {
  return (
 ...
 * - TRANSACTION: up to MAX_RECENT_TRANSACTIONS most recent BUY/SELL cards,
 *   merged across every instance of this ticker, plus a "See all" drawer
 *   with the full history table. No add/edit UI yet — auto-populating a
 *   TRANSACTION-mode dividend from market data is planned as a separate
 *   scheduled job, not built here (see the parked auto-populate feature).
 *
...
function HoldingTransactionsSection({
  ...
  const recentWithLocation = recent.map((transaction) => ({
    transaction,
    location: allTransactions.find((entry) => entry.transaction.id === transaction.id)?.location,
  }));
    ...
    <Card className="ec-detail-section">
      ...
      {recent.length > 0 ? (
        <div className="ec-dividend-grid">
          {recentWithLocation.map(({ transaction, location }) => (
            <TradeCard
              key={transaction.id}
              transaction={transaction}
              nativeCurrency={nativeCurrency}
      ...
      <Drawer open={drawer === "see-all"} onClose={closeDrawer} title="Transactions">
        <div className="ec-table-wrap">
          <table className="ec-table">
            ...
            <tbody>
              {fullHistory.map(({ transaction, location }) => (
                <tr key={transaction.id}>
                  <td>
