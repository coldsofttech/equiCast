 ...
 * fx, plus benchmark/future when explicitly asked for — see below). Used
 ...
 * `assetClass`/`page` for its Type filter, including "Benchmark Index" and
 * "Futures" — see SearchFilters.jsx — and "Load more"), by
 ...
 * searches `fx`/`stock`/`etf` only — `benchmark`/`future` are opt-in-only,
 * never part of an unfiltered search (see
 * `equicast_core.client.DEFAULT_SEARCH_ASSET_CLASSES`), so TopbarSearch's
 * quick-search and SearchPage's "All types" never surface either; both are
 * still reachable deliberately, though — `benchmark` via
 * HoldingComparePicker's own explicit search, and both `benchmark` and
 * `future` via SearchPage's own Type filter (see SearchFilters.jsx), which
 * offers them explicitly even though neither appears unasked.
 * `minMarketCap`/`maxMarketCap` (SearchFilters'
