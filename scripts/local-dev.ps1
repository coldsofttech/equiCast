<#
.SYNOPSIS
  Starts LocalStack (S3 + DynamoDB), provisions the buckets/table the
  Django backend needs, and runs the backend against them - a fully local
  stand-in for MARKET_DATA_BUCKET/USER_DATA_BUCKET/USER_PROFILES_TABLE,
  with no real AWS account (or LocalStack account) required.
...
.DESCRIPTION
  ...
  Run `cd frontend; npm run dev` in another terminal and open
  http://localhost:5173 to drive this from the browser - this script only
  starts LocalStack + the backend, not the frontend dev server.

  Ctrl+C stops the backend and also tears down the LocalStack container
  (a fresh one is created next run - no data persists across sessions).

.PARAMETER SeedMarketData
  Also ingests all three asset classes via their CLI/config
  (equicast-fx/packages/fx/config/fx_pairs.yaml,
...
.PARAMETER Reset
  Removes any existing LocalStack container (and its data) before
  starting a fresh one.

.PARAMETER Stop
  Stops and removes the LocalStack container, then exits without touching
  the backend.

.EXAMPLE
  .\scripts\local-dev.ps1
.EXAMPLE
  .\scripts\local-dev.ps1 -SeedMarketData
.EXAMPLE
...
# --- 2. Wait for S3 + DynamoDB to be ready --------------------------------
...
# --- 3. Fake creds + boto3 endpoint overrides -----------------------------
# equicast-core's clients call plain boto3.client(...)/boto3.resource(...)
# with no endpoint_url hook of their own - this relies on boto3>=1.28's
# AWS_ENDPOINT_URL_* env var support (pinned boto3>=1.35.9 here) to route
# them at LocalStack instead, with no code change.
...
# --- 4. Provision the S3 buckets and DynamoDB table -----------------------
...
# --- 5. Optionally seed all three asset classes' catalogs -----------------
if ($SeedMarketData) {
    if (-not (Test-Command uv)) {
        Write-Error "uv is required for -SeedMarketData (runs the equicast-fx/equicast-stock/equicast-etf/equicast-core CLIs). Install it: https://docs.astral.sh/uv/"
        exit 1
    }
...
# --- 6. Auth0 (optional - not simulated by LocalStack) --------------------
...
# --- 7. Run the backend ----------------------------------------------------
Write-Host ""
Write-Host "Starting the Django dev server against LocalStack:"
...
Write-Host ""
Write-Host "In another terminal: cd frontend; npm run dev -- then open http://localhost:5173"
Write-Host "Press Ctrl+C to stop the backend and tear down LocalStack."
Write-Host ""

if (-not (Test-Command uv)) {
    Write-Error "uv is required to run the backend. Install it: https://docs.astral.sh/uv/"
    exit 1
}
