...
- Consolidated `missing-isin-fix.yml` and the new config-change fix logic into one shared `issue-automation-fix.yml` workflow, routed per scenario on the dispatch's event type, and merged their per-scenario branch-cleanup workflows into one `cleanup-merged-automation-branches.yml` — same behavior, gates, and PR/branch shape per scenario, so a future scenario plugs into the existing workflow pair instead of getting its own.
...
- Missing-ISIN GitHub issue notification (GitHub issue #215):
  ...
  and `.github/scripts/sync-missing-isin-issue.sh`, finds-or-creates a
  persistent parent GitHub issue per pipeline ("Stock ingestion: tickers
  missing ISIN" / "ETF ingestion: tickers missing ISIN") and opens a native
  GitHub sub-issue per ticker with no ISIN on record, reopening/closing
  each independently as it goes missing/gets resolved — so you can add an
  `isin`/`tax_domicile` override to `stocks.prod.yaml`/`etfs.prod.yaml` and
  close the sub-issue via a fix PR (`Closes #<sub-issue>`). Uses GitHub's
  own issue notification emails rather than a separate email/SMTP
  integration.
