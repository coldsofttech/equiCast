...
- Top Winners and Top Losers are now live system watchlists, ranking the
  ...
  movers is separate, not-yet-built work). `movers.compute_cagr_rankings`
  gained the same per-ticker failure isolation as `builder.build_entries`
  (equicast-support#145) — one bad ticker out of the whole stock/ETF
  universe no longer loses the entire ranking; since Global Markets/the
  ranking/Top Winners/Top Losers all now share one job's filesystem
  instead of separate matrix legs, each gets its own "set aside failures
  manifest" step before the next one can overwrite it.
