...
def test_build_top_movers_losers_keeps_only_negative_cagr_lowest_first() -> None:
    ...
    assert [row["ticker"] for row in losers] == ["B", "A"]
    assert losers[0]["change_1y_pct"] == -30.0


def test_build_top_movers_respects_limit() -> None:
    rows = [
        {**_catalog_row(f"T{i}"), "asset_class": "stock", "cagr_1y": float(i)} for i in range(1, 6)
    ]
...
def test_build_account_movers_excludes_holdings_with_no_matching_or_qualifying_row() -> None:
    stock_etf_rows = [
