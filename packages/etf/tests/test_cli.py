...
def test_run_writes_profile_price_dividend_events_metrics_and_news_parquet_per_configured_ticker(
    ...
    for ticker in ("VOO", "QQQ"):
        ...
        assert (out_dir / f"etf={ticker}" / "price" / "current.parquet").exists()
        assert (out_dir / f"etf={ticker}" / "dividend" / "current.parquet").exists()
...
def test_run_omits_news_parquet_when_no_news_published(tmp_path: Path) -> None:
    ...
    assert not (out_dir / "etf=VOO" / "news.parquet").exists()
...
def test_run_accepts_tickers_json_instead_of_config(tmp_path: Path) -> None:
    ...
    assert set(written) == {
        ...
        out_dir / "etf=VOO" / "price" / "current.parquet",
        out_dir / "etf=VOO" / "dividend" / "current.parquet",
        out_dir / "etf=VOO" / "events" / "current.parquet",
        out_dir / "etf=VOO" / "metrics.parquet",
        out_dir / "etf=VOO" / "news.parquet",
    }
...
def test_run_shares_one_datafeed_client_across_workers(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "equicast_etf.cli.EventsClient", side_effect=_fake_events_client_factory()
        ) as mock_events_client,
        patch(
            "equicast_etf.cli.MetricsClient", side_effect=_fake_metrics_client_factory()
        ) as mock_metrics_client,
        patch(
            "equicast_etf.cli.NewsClient", side_effect=_fake_news_client_factory()
        ) as mock_news_client,
    ):
