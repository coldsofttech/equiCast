...
`day_*`/`year_*` mirror `equicast-fx`'s `FXClient.profile()` exactly (same
source fields, same logic): `day_close` is the live price; `year_*` uses a
...
All rounded to 8 decimal places (`equicast-datafeed`'s `round_value`),
`None` wherever the underlying yfinance field is missing.
