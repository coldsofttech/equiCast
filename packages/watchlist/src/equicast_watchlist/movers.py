...
def compute_cagr_rankings(
    ...
    max_workers: int = 1,
) -> tuple[list[TickerCagr], list[dict[str, str]]]:
    """Step 1: every stock/ETF ticker's trailing 1-year CAGR, in config order
    (stocks then ETFs) regardless of fetch completion order — futures are
    submitted and then `.result()`-ed back in that same original order
    rather than via `as_completed`, same ordering guarantee as
    `builder.build_entries`.

    A ticker whose `MetricsClient.metrics()` call raises outright (yfinance
    down, a bad symbol) is caught here, logged, and recorded in the
    returned failures list rather than aborting every other ticker's
    ranking too — the same per-item resilience equicast-support#145 added
    to every other ingestion pipeline's `cli.run()`, applied here since a
    single bad ticker out of the whole stock/ETF universe would otherwise
    lose the entire Top Winners/Top Losers ranking, not just that ticker.
    """
    universe = _universe(stock_config, etf_config)
    failures: list[dict[str, str]] = []

    def _record_failure(ticker: str, exc: Exception) -> None:
        logger.exception("Failed to rank ticker %s", ticker)
        failures.append({"ticker": ticker, "task": "metrics", "error": str(exc)})

    rankings: list[TickerCagr] = []
    if max_workers <= 1:
        for asset_class, ticker in universe:
            try:
                rankings.append(_rank_one(asset_class, ticker, datafeed))
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(_rank_one, asset_class, ticker, datafeed)
            for asset_class, ticker in universe
        ]
        for (_, ticker), future in zip(universe, futures, strict=True):
            try:
...
def select_top(rankings: list[TickerCagr], direction: str, limit: int) -> list[TickerCagr]:
    ...
    if direction == "winners":
        ...
        candidates.sort(key=lambda r: cast(float, r.cagr_1y), reverse=True)
    elif direction == "losers":
        candidates = [r for r in rankings if r.cagr_1y is not None and r.cagr_1y < 0]
        candidates.sort(key=lambda r: cast(float, r.cagr_1y))
    else:
...
def merge_change_1y(
    ...
    for row in rows:
        cagr_1y = cagr_by_ticker.get(str(row["ticker"]))
