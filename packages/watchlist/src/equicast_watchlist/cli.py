...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        help="Path to this watchlist's entries YAML. Required for --mode config.",
    )
    parser.add_argument(
        "--stock-config",
        type=Path,
        help="Path to equicast-stock's tickers YAML. Required for --mode rank.",
    )
    parser.add_argument(
        "--etf-config",
        type=Path,
        help="Path to equicast-etf's tickers YAML. Required for --mode rank.",
    )
    parser.add_argument(
        "--direction",
    ...
    parser.add_argument(
        "--rankings",
        type=Path,
        help="Path to --mode rank's output JSON. Required for --mode movers.",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def run(
    ...
    built, failures = build_entries(entries, datafeed, max_workers=max_workers)
...
def run_rank(
    ...
    written to `<output_dir>/rankings.json`. A failed ticker is recorded in
    `<output_dir>/failures.json` instead of aborting the whole ranking -
    see `movers.compute_cagr_rankings`."""
    datafeed = DatafeedClient(max_calls=max_calls, period_seconds=period_seconds)
    rankings, failures = compute_cagr_rankings(
        stock_config, etf_config, datafeed, max_workers=max_workers
    )
    write_failures_manifest(failures, output_dir)
...
def run_movers(
    ...
    (including the same per-entry failure isolation/failures.json)."""
