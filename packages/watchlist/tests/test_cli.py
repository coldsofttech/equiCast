...
def test_run_rank_writes_rankings_json(tmp_path: Path) -> None:
    ...
    with (
        patch("equicast_watchlist.cli.DatafeedClient") as mock_datafeed_cls,
        patch(
            "equicast_watchlist.cli.compute_cagr_rankings",
    ...
    mock_rank.assert_called_once_with(
        stock_config, etf_config, mock_datafeed_cls.return_value, max_workers=1
    )


def test_run_rank_writes_a_failures_manifest_when_a_ticker_fails(tmp_path: Path) -> None:
    stock_config = tmp_path / "stocks.yaml"
    stock_config.write_text("tickers:\n  - AAPL\n")
    etf_config = tmp_path / "etfs.yaml"
    etf_config.write_text("tickers:\n  - VOO\n")
    out_dir = tmp_path / "rankings"
    failures = [{"ticker": "AAPL", "task": "metrics", "error": "yfinance boom"}]

    with (
        patch("equicast_watchlist.cli.DatafeedClient"),
        patch("equicast_watchlist.cli.compute_cagr_rankings", return_value=([], failures)),
    ):
        run_rank(stock_config, etf_config, out_dir)
...
def test_run_movers_writes_a_failures_manifest_when_build_entries_reports_failures(
    tmp_path: Path,
) -> None:
    rankings_path = tmp_path / "rankings.json"
    rankings_path.write_text('[{"asset_class": "stock", "ticker": "AAPL", "cagr_1y": 0.2}]')
    out_dir = tmp_path / "output"
    failures = [{"ticker": "AAPL", "task": "profile", "error": "yfinance boom"}]

    with (
        patch("equicast_watchlist.cli.DatafeedClient"),
        patch("equicast_watchlist.cli.build_entries", return_value=([], failures)),
    ):
        run_movers("TOP_WINNERS", "winners", rankings_path, 50, out_dir)
