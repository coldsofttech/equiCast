...
def test_build_entries_preserves_config_order_with_multiple_workers() -> None:
    ...
    with patch("equicast_watchlist.builder.FutureClient", side_effect=fake_client):
        rows, failures = build_entries(entries, datafeed, max_workers=4)

    assert [row["ticker"] for row in rows] == [f"F{i}" for i in range(10)]
    assert failures == []


def test_build_entries_isolates_a_failed_entry_single_worker() -> None:
    datafeed = _datafeed()
    ...
    def fake_client(key, symbol, datafeed=None):
        ...
        client.symbol = symbol
        if key == "BAD":
            client.profile.side_effect = RuntimeError("yfinance boom")
        else:
            client.profile.return_value = {
                "name": key,
                "currency": "USD",
    ...
    entries = [
        WatchlistEntry(asset_class="future", ticker="GOOD", name="Good", key="GOOD", symbol="G=F"),
        WatchlistEntry(asset_class="future", ticker="BAD", name="Bad", key="BAD", symbol="B=F"),
    ]

    with patch("equicast_watchlist.builder.FutureClient", side_effect=fake_client):
        rows, failures = build_entries(entries, datafeed, max_workers=1)

    assert [row["ticker"] for row in rows] == ["GOOD"]
    assert failures == [{"ticker": "BAD", "task": "profile", "error": "yfinance boom"}]


def test_build_entries_isolates_a_failed_entry_multiple_workers() -> None:
    datafeed = _datafeed()
    ...
    assert [row["ticker"] for row in rows] == [f"F{i}" for i in range(5)]
    assert failures == [{"ticker": "BAD", "task": "profile", "error": "yfinance boom"}]
