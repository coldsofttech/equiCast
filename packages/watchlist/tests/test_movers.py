...
def test_compute_cagr_rankings_covers_stock_then_etf_tickers_in_order(tmp_path: Path) -> None:
    ...
    with patch("equicast_watchlist.movers.MetricsClient") as mock_metrics_client:
        ...
        rankings, failures = compute_cagr_rankings(stock_config, etf_config, MagicMock())
...
def test_compute_cagr_rankings_preserves_order_with_multiple_workers(tmp_path: Path) -> None:
    ...
    with patch("equicast_watchlist.movers.MetricsClient", side_effect=fake_metrics_client):
        rankings, failures = compute_cagr_rankings(
            stock_config, etf_config, MagicMock(), max_workers=4
        )
...
def test_compute_cagr_rankings_isolates_a_failed_ticker_single_worker(tmp_path: Path) -> None:
    stock_config = _write_yaml(tmp_path / "stocks.yaml", ["GOOD", "BAD"])
    etf_config = _write_yaml(tmp_path / "etfs.yaml", [])

    def fake_metrics_client(ticker, datafeed=None):
        client = MagicMock()
        if ticker == "BAD":
            client.metrics.side_effect = RuntimeError("yfinance boom")
        else:
            client.metrics.return_value = {"cagr_1y": 0.1}
        return client

    with patch("equicast_watchlist.movers.MetricsClient", side_effect=fake_metrics_client):
        rankings, failures = compute_cagr_rankings(stock_config, etf_config, MagicMock())

    assert [r.ticker for r in rankings] == ["GOOD"]
    assert failures == [{"ticker": "BAD", "task": "metrics", "error": "yfinance boom"}]


def test_compute_cagr_rankings_isolates_a_failed_ticker_multiple_workers(tmp_path: Path) -> None:
    stock_config = _write_yaml(tmp_path / "stocks.yaml", [f"T{i}" for i in range(5)] + ["BAD"])
    etf_config = _write_yaml(tmp_path / "etfs.yaml", [])
    ...
    assert [r.ticker for r in rankings] == [f"T{i}" for i in range(5)]
    assert failures == [{"ticker": "BAD", "task": "metrics", "error": "yfinance boom"}]
