...
class MarketDataClient:
    ...
    def search(
        ...
        `None` for fx/benchmark/future rows), across `asset_classes`
        (default: `DEFAULT_SEARCH_ASSET_CLASSES` — `benchmark`/`future` are
        scanned only when a caller explicitly asks for one, e.g.
