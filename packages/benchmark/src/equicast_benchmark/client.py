"""Class-based client for a single benchmark (market index), backed by
yfinance via equicast-datafeed.

A benchmark is a single yfinance symbol (like a stock ticker), not a pair
(like FX) — so its shape mirrors `equicast_fx.FXClient` closely (no
dividends, no fundamentals; an index pays none and has no earnings/balance
sheet), just keyed by a single `key`/`symbol` instead of a
...
#: `prices(full_load=True)` window: everything yfinance has for this benchmark.
...
class BenchmarkClient:
    """Fetches profile and price data for one benchmark (market index)."""
    ...
    def profile(self) -> dict[str, Any]:
        """Return profile and price-range data for this benchmark."""
    ...
    def prices(self, full_load: bool = False) -> list[dict[str, Any]]:
        ...
        `full_load=True`, covers this benchmark's entire yfinance history
        instead.
        """
