...
def write_failures_manifest(failures: list[dict[str, str]], output_dir: Path) -> Path | None:
    ...
    benchmark/task that raised during this run - see `cli.run`; the key is
    literally `"ticker"` even though this pipeline's items are benchmark
    keys, so every ingestion package's failures.json shares one schema for
    ...
    empty, same convention as the optional per-benchmark parquet writers
    above.
    """
    if not failures:
        return None
