...
def test_run_continues_past_a_failed_future_task_and_records_a_failures_manifest(
    tmp_path: Path,
) -> None:
    config = tmp_path / "futures.yaml"
    config.write_text(
    ...
    def fake_future_client(key: str, symbol: str, datafeed=None) -> MagicMock:
        client = _fake_future_client_factory()(key, symbol, datafeed=datafeed)
        if key == "SILVER":
    ...
    with (
        patch("equicast_future.cli.DatafeedClient"),
        patch("equicast_future.cli.FutureClient", side_effect=fake_future_client),
        patch("equicast_future.cli.MetricsClient", side_effect=_fake_metrics_client_factory()),
    ):
        written = run(config, out_dir, max_workers=2)

    # SILVER's failed prices task doesn't stop GOLD's (or SILVER's own
    # other) tasks from completing and being written.
    assert out_dir / "future=GOLD" / "price" / "current.parquet" in written
    assert out_dir / "future=SILVER" / "profile.parquet" in written
    assert not (out_dir / "future=SILVER" / "price" / "current.parquet").exists()

    failures = json.loads((out_dir / "failures.json").read_text(encoding="utf-8"))
    assert failures == [{"ticker": "SILVER", "task": "prices", "error": "yfinance boom"}]
