...
def test_write_profile_parquet_partitions_by_key(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / "future=GOLD" / "profile.parquet"
...
def test_write_price_parquet_splits_into_history_and_current(tmp_path: Path) -> None:
    ...
    assert set(paths) == {
        tmp_path / "future=GOLD" / "price" / "history.parquet",
        tmp_path / "future=GOLD" / "price" / "current.parquet",
    }

    history = pd.read_parquet(tmp_path / "future=GOLD" / "price" / "history.parquet")
    assert sorted(history["date"]) == ["2025-12-30", "2025-12-31"]

    current = pd.read_parquet(tmp_path / "future=GOLD" / "price" / "current.parquet")
...
def test_write_price_parquet_current_year_only_writes_no_history_file(tmp_path: Path) -> None:
    ...
    assert paths == [tmp_path / "future=GOLD" / "price" / "current.parquet"]
    assert not (tmp_path / "future=GOLD" / "price" / "history.parquet").exists()
...
def test_write_metrics_parquet_adds_key(tmp_path: Path) -> None:
    ...
    path = write_metrics_parquet(metrics, "GOLD", tmp_path)

    assert path == tmp_path / "future=GOLD" / "metrics.parquet"
    result = pd.read_parquet(path)
    assert result.to_dict(orient="records") == [{"key": "GOLD", **metrics}]


def test_write_failures_manifest_writes_json(tmp_path: Path) -> None:
    failures = [{"ticker": "GOLD", "task": "prices", "error": "boom"}]
