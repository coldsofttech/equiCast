...
from equicast_future.plan import chunk_futures, filter_futures
...
def test_filter_futures_returns_unchanged_when_keys_is_none() -> None:
    futures = _futures(3)
    assert filter_futures(futures, None) == futures


def test_filter_futures_restricts_to_matching_keys() -> None:
    futures = [
        Future(key="GOLD", symbol="GC=F"),
        Future(key="SILVER", symbol="SI=F"),
    ]

    assert [f.key for f in filter_futures(futures, "SILVER")] == ["SILVER"]


def test_filter_futures_is_case_insensitive_and_ignores_blank_entries() -> None:
    futures = [
        Future(key="GOLD", symbol="GC=F"),
        Future(key="SILVER", symbol="SI=F"),
    ]

    assert [f.key for f in filter_futures(futures, " gold ; ;silver ")] == ["GOLD", "SILVER"]
...
def test_plan_cli_restricts_to_given_tickers(
    ...
    output = json.loads(capsys.readouterr().out)
    assert output == [[{"key": "SILVER", "symbol": "SI=F"}]]
