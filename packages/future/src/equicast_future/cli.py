...
def run(
    ...
    for future in futures:
        ...
        tasks.append(partial(_profile_task, client, output_dir, future.key))
        tasks.append(partial(_prices_task, client, output_dir, future.key, full_load))
        tasks.append(partial(_metrics_task, metrics_client, future.key, output_dir))

    written: list[Path] = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures_submitted = [executor.submit(task) for task in tasks]
        for done in as_completed(futures_submitted):
