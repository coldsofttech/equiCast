...
def test_write_price_parquet_splits_into_history_and_current(tmp_path: Path) -> None:
    ...
    assert set(paths) == {
        tmp_path / "fx=GBPUSD" / "price" / "history.parquet",
        tmp_path / "fx=GBPUSD" / "price" / "current.parquet",
    }

    history = pd.read_parquet(tmp_path / "fx=GBPUSD" / "price" / "history.parquet")
    assert sorted(history["date"]) == ["2025-12-30", "2025-12-31"]

    current = pd.read_parquet(tmp_path / "fx=GBPUSD" / "price" / "current.parquet")
...
def test_write_price_parquet_current_year_only_writes_no_history_file(tmp_path: Path) -> None:
    ...
    assert paths == [tmp_path / "fx=GBPUSD" / "price" / "current.parquet"]
    assert not (tmp_path / "fx=GBPUSD" / "price" / "history.parquet").exists()
